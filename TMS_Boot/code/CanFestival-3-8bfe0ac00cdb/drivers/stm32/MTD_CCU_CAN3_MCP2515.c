...
{		
	...
	while(1)
	{
		...
		{
			DEBUG_PRINT("MERRF报文错误中断！\r\n");
							//MTDCCU_CAN3_MCP2515_IntError_Init(MCP2515_CAN_BAUD_125K);	//复位CAN3  //W HH  /*WHH*/
			MTDCCU_CAN3_MCP2515_Register_BitModify(CANINTF,MERRF,0);	// 清报文错误中断标志
		}
		
		if(state & WAKIF) // 唤醒中断
		{		
			DEBUG_PRINT("WAKIF唤醒中断！\r\n");
